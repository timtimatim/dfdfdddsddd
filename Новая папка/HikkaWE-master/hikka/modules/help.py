...
class HelpMod(loader.Module):
    ...
    strings = {
        ...
        "request_join": "You requested link for Hikka WE support chat",
        "core_notice": (
    ...
    strings_ru = {
        ...
        "request_join": "Вы запросили ссылку на чат помощи Hikka WE",
        "core_notice": (
    ...
    @loader.command(
        ru_doc="Показать ссылку на чат помощи Hikka WE",
    )
    async def support(self, message):
        """Get link of Hikka WE support chat"""
        if message.out:
            await self.request_join("@hikkawe_support", self.strings("request_join"))
