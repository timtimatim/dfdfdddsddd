...
def get_named_platform() -> str:
    ...
    # windows edition
    return f"<emoji document_id=5370869711888194012>👾</emoji> {platform.system()} {platform.release()}"
...
def get_platform_emoji(client: typing.Optional[CustomTelegramClient] = None) -> str:
    ...
    if "Windows 7" in get_named_platform():
        return BASE.format(5208767321035778430)

    if "Windows 8" in get_named_platform():
        return BASE.format(5208477208879833656)

    if "Windows 8.1" in get_named_platform():
        return BASE.format(5208477208879833656)

    if "Windows 10" in get_named_platform():
        return BASE.format(5208477208879833656)

    if "Windows 11" in get_named_platform():
        return BASE.format(5208665594735370734)
